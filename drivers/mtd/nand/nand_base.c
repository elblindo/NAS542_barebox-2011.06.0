...
#if 0
	// read the header(16 bytes) of kernel1 partition to determine if it's empty.
//printf("@@@@@ ");
	for (i = 0; i < 16; i++)
	{
//		printf("%2X ", buf[i]);
	...
	if (j == 16)
	{
		// kernel1 partition is empty, set a flag "/env/empty_nand"
		fd = open("/env/empty_nand", O_CREAT|O_WRONLY);
		close(fd);
	}
//printf("@@@@@\n");
#endif
